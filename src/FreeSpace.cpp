...
FreeSpace *FreeSpace::pushBeginningRight(byte *firstByte) {
    ...
    if (firstByte ==
        ...
        assert(CodeBlock::readFromLeft(firstByte) < 8 ||
               *(newFreeSpace->getLeftNext(CodeBlock::getBlockSize(firstByte)))
...
FreeSpace *FreeSpace::pushEndLeft(byte *lastByte) {
    ...
    CodeBlock::getCodeBlockForInternalSize(getLeftMostEnd(), (lastByte - getLeftMostEnd()) + 1, codeBlockSize,
                                           true);//get the needed size
    copyCodeBlockToEnd(
            getLeftMostEnd(),
            codeBlockSize
    );
    ...
    assert(CodeBlock::readFromLeft(getLeftMostEnd()) < 8
           || *getLeftNext(CodeBlock::getBlockSize(getLeftMostEnd())) ==
              *getRightNext(CodeBlock::getBlockSize(getLeftMostEnd())));
#endif
...
uint32_t *FreeSpace::getRightNext(size_t codeBlockSize) {
    return (uint32_t *) ((getRightMostEnd() - (codeBlockSize)) - sizeof(uint32_t) +
                         1);//uint32_t is 4 byte in contrast to the one byte rightMostEnd pointer
}
...
void FreeSpace::writeNextPointer(uint32_t nextPointer, byte *leftCodeBlock) {
    ...
    *leftNext = nextPointer;
    ...
    assert(CodeBlock::readFromLeft(leftCodeBlock) ==
           CodeBlock::readFromLeft(leftCodeBlock + spaceSize + codeBlockSize));
