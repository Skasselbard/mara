...
void Page::initializeBucketList() {
    ...
    assert(CodeBlock::readFromLeft((byte *) bucketList.getFromBucketList(bucketList.blSize - 1)) ==
...
OccupiedSpace *Page::getDynamicBlock(size_t sizeInByte) {
    ...
    if (freeSpace == nullptr) {
        ...
        FreeSpace *remainingSpace = cutLeftFromFreeSpace(freeSpace,
                                                         sizeInByte +
                                                         (2 * CodeBlock::getNeededCodeBlockSize(sizeInByte)));
        if (remainingSpace) {
...
FreeSpace *Page::cutRightFromFreeSpace(FreeSpace *freeSpace, size_t bytesToCutOf) {
    ...
    assert(freeSpace >= startOfPage &&
           (byte *) freeSpace < (byte *) startOfPage + pageSize);//the freespace must be in the page
#endif
    if ((freeSpace->getSize() - bytesToCutOf) < SMALLEST_POSSIBLE_FREE_SPACE) {
#ifdef POSTCONDITION
        //see if clause
...
#ifdef POSTCONDITION
    assert((leftNeighbor == nullptr && bucketList.searchInList((FreeSpace *) codeBlockStart) &&
